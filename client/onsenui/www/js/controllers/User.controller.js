angular
  ...
  .controller('UserListController', ['$scope', 'User', function($scope,
    ...
    $scope.showDetail = function(id) {
      myNavigator.pushPage('/onsenui/www/views/users/detail.html', { animation : 'slide', recordId: id } );
    };
  }])
  .controller('UserDetailController', ['$scope', 'User', function($scope,
    ...
    $scope.record = User.findById({id: recordId});

    $scope.edit = function() {
      myNavigator.pushPage('/onsenui/www/views/users/edit.html', { animation : 'lift', recordId: recordId } );
    };
  ...
  .controller('UserEditController', ['$scope', 'User', '$rootScope',
    ...
    $scope.save = function() {
      ...
      User        .update({where: {id: recordId}}, requestParams)
        .$promise
        .then(function(record) {
          myNavigator.popPage({ recordId: record.id });
        });
